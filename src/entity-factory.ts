...
export abstract class EntityFactory<E> {
  /**
   * @constructor
   * @property em: MikroORM Entity Manager
  ...
  async saveMany(count = 1, overrides?: Partial<E>): Promise<E[]> {
    ...
    for await (const index of [...Array(count).keys()]) {
       ...
       */
      if (overrides) {
        const entityOverrides: Partial<E> = <Partial<E>>{ ...overrides };
        Object.keys(entity).forEach(key => {
          /** Grab the value at override[key] which is of type E[key] */
          const overrideValue: E[keyof E] | undefined = entityOverrides[
            key as keyof E
          ]!;
    ...
    await this.em.persistAndFlush(entityArray);
    return entityArray;
  }
